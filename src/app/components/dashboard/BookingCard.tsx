...
export default function BookingCard({ booking }: Props) {
  ...
  const [inquiryCompleted, setInquiryCompleted] = useState(
  ...
  const handleCancel = async () => {
    ...
    // Refresh the page so PatientRequests reloads the requests
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      const response = await fetch("/api/patient-requests/accept", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "We could not accept this request."
        );
      }
  ...
  function getRequestStatus(booking: Booking) {
    if (booking.clinicDecision && booking.patientDecision) {
      return {
        label: "Accepted",
...
<div className="flex gap-3">
  ...
  {booking.status === "pending" && !inquiryCompleted && (
  <button
    type="button"
...
{booking.status === "pending" && inquiryCompleted && !booking.clinicDecision && (
  <button
    type="button"
    onClick={() => setInquiryOpen(true)}
    className="px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:opacity-90 transition-opacity"
...
{booking.clinicDecision && patientDecision && (
  <div className="mt-4">
    <button
      onClick={() => setExpanded((s) => !s)}
      className="text-sm font-medium text-primary hover:underline"
    >
      {expanded ? "Hide" : "Manage Booking"}
    </button>
      ...
      <div className="mt-3 space-y-4">
        <div className="border border-border rounded-lg p-4">
          ...
          <AccommodationTab
            ...
            booking={localBooking as any}
            setBooking={setLocalBooking as any}
          />
        </div>
